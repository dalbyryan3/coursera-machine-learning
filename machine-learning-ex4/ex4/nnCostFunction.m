...
% Convert y to vectors of 1's and 0's
yv = zeros(m, num_labels);
for i = 1:m
    yv(i, y(i)) = 1;
end

% Forward prop to determine h
a1 = [ones(m,1), X];
z2 = a1 * Theta1';
...
% compute cost by summing over each training example
% h is m by num_labels 
% yv is m by num_labels
J = (1/m) * sum(((-yv .* log(h))-((1 - yv) .* log(1 - h))), 'all'); 

% Regularization of cost function
J = J + (lambda/(2*m)) * (sum((Theta1(:,2:end).^2),'all') + sum((Theta2(:,2:end).^2),'all'));

% Backprop for each training example (loopwise)
% for t = 1:m
%     delta_3 = a3(t,:)' - yv(t,:)'; 
...
% Complete vectorization, implicitly loop over training examples
delta_3 = a3' - yv';
